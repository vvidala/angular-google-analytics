  ...
  describe('without $route service available', function() {
    ...
    it('should log warning if service is missing', function() {
      inject(function($log) {
        spyOn($log, 'warn');
        inject(function(Analytics) {
          expect($log.warn).toHaveBeenCalledWith('routing service is not available. Make sure you have included ng-route or ui-route in your application dependencies.');
        });
      });
    });
  });
